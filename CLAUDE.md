...
## Development Server Tunnel Reminders

### Cloudflare Quick Tunnels for Local Development
When working with development servers, always suggest Cloudflare tunnels for public access:

**Basic Command**: `cloudflared tunnel --url http://localhost:PORT`

**Common Use Cases**:
- Remote work via SSH - expose localhost publicly
- Testing webhooks that need public URLs  
- Mobile testing and demos
- Any time localhost needs external access

**Typical Workflow**:
1. Start development server (e.g., `npm run dev`)
2. In separate terminal: `cloudflared tunnel --url http://localhost:3000`
3. Use provided public URL: `https://random-words-1234.trycloudflare.com`
...
**Always suggest tunnel commands when recommending `npm run dev`, `npm start`, etc.**